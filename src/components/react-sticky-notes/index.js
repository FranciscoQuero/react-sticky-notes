...
class ReactStickyNotes extends Component {
  ...
  dispatch = (options) => {
    ...
    this.setState(reducer(this.state, { type, payload }), () => {
  ...
  getColor() {
    return this.props.colorCodes[
      Math.floor(Math.random() * this.props.colorCodes.length)
    ];
  }
  addItem = (e, data) => {
    const { items } = this.state;
    const index = data
      ? items.findIndex((item) => item.id === data.id) + 1
      : items.length;
    this.dispatch({
      type: "add",
      payload: {
